<?php 
 ...
 function vendas($usuario,$products){
 	$CI =& get_instance();
        ...
        //     'peso' => 0
        // );
        $CI->pagseguro->set_products($products);
        
        // ID do pedido
        $config['reference'] = rand(999, 9999);

        // gera botão
        //echo $CI->pagseguro->get_button($config);
        return $CI->pagseguro->get_button($config);
        ...
        if (count($_POST) > 0) {
            ...
            $CI->load->library('pagseguro'); //Carrega a library
            
            
            // faz conexão com PS para validar o retorno
            $retorno = $CI->pagseguro->notificationPost();
            ...
            if ($notificationType && $notificationCode) {
                ...
                $not = $CI->pagseguro->get_notification($notificationCode);
                /*
    ...
    function check($code = NULL) {        
        ...
        $string = $CI->pagseguro->get_notification($code);
